...
class SceneGraphConstructor:
    ...
    def construct_scene_graphs(self, scene_graph, category_dict, colour_dict) :
        ...
        for object_dict in scene_graph['objects']:
            new_feature_vector = self.make_object_node_feature_vector(object_dict, category_dict, colour_dict)
        ...
        for predicate in scene_graph['relationships']:
            ...
            object_vector, object_index = next(((vec, i) for i, vec in enumerate(all_feature_vectors) if vec[6] == object_id), (None, None))
            subject_vector, subject_index = next(((vec, i) for i, vec in enumerate(all_feature_vectors) if vec[6] == subject_id), (None, None))

          
            # Constructing the predicate node feature vector
            predicate_node = self.make_predicate_node_feature_vector(object_vector = object_vector,
    ...
    @staticmethod 
    def make_object_node_feature_vector(object_dict, all_node_categories,
    all_colour_categories):
    ...
    # all_colour_categories: dict) -> torch.Tensor:
        ...
        new_feature_vector = torch.tensor([x, y, h, w, category_id, colour_id, object_id], dtype=torch.float)

        return new_feature_vector

    @staticmethod 
    def make_predicate_node_feature_vector(subject_vector, object_vector, 
                                           predicate_dict, all_node_categories):
    ...
    #                                        predicate_dict: dict, all_node_categories: dict) -> torch.Tensor:
        ...
        new_feature_vector = torch.tensor([x, y, w, h, category_id, colour_id, predicate_id], dtype=torch.float)
